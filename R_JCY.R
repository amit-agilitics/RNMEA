...
p=ggplot(data=Mytable,aes(Time_,Rudder_Angle$class, fill=positive))+geom_bar(stat='identity', position = 'identity')+ scale_fill_manual(values=c("red", " dark green"))+theme(axis.text.x = element_text(angle = 0),axis.text.y = element_text(angle = 0),legend.position='none')+ggtitle(paste("Rudder Angle vs Time","(from ", min_t, " to ",max_t,")" ))+ scale_x_datetime(labels=date_format("%H:%M"),breaks=date_breaks("60 min"),name ="Time in UTC")+scale_y_continuous(name ="Rudder Angle (Degrees)", limits=c(-40,40), breaks=seq(-40,40, by = 10))
grob1 <- grobTree(textGrob("Starboard", x=0.1,  y=0.95, hjust=0,
                           gp=gpar(col="dark gREEN", fontsize=13, fontface="italic")))
grob2 <- grobTree(textGrob("Port", x=0.2,  y=.95, hjust=0,vjust = 25,
                           gp=gpar(col="red", fontsize=13, fontface="italic")))
grob3 <- grobTree(textGrob(mintime, x=0.4,  y=.95, hjust=0,vjust = 32,
                           gp=gpar(col="red", fontsize=13, fontface="italic")))
grob4 <- grobTree(textGrob("-", x=0.65 , y=.95, hjust=0,vjust = 32,
                           gp=gpar(col="red", fontsize=13, fontface="italic")))
grob5 <- grobTree(textGrob(maxtime, x=0.7,  y=.95, hjust=0,vjust = 32,
                           gp=gpar(col="red", fontsize=13, fontface="italic")))
p+ theme( panel.grid.minor = element_line(colour = "gray", linetype = "dotted",size=.5))+annotation_custom(grob1)+annotation_custom(grob2)+annotation_custom(grob3)+annotation_custom(grob4)+annotation_custom(grob5)+geom_hline(yintercept=0)
...
ggplot(data=Mytable,aes(Time_,DPT$class))+geom_line(color="Blue",aes(group=1)) +theme(axis.text.x = element_text(angle = 0),axis.text.y = element_text(angle = 0))+ggtitle(paste("UKC vs Time","(from ", min_t, " to ",max_t,")" ))+ scale_x_datetime(labels=date_format("%H:%M"),breaks=date_breaks("60 min"),name ="Time in UTC")+theme(panel.grid.minor = element_line(colour = "gray", linetype = "dotted",size=.5))+scale_y_continuous(name ="Depth in Meters",limits=c(0,80), breaks=seq(0,80, by = 10))
ggsave("UKCvsTime.png")

#True Heading
Mytable=sqldf('Select * from Data where V3 like "%HDT"')
Time_<-strptime(Mytable$V1,"%Y/%m/%d %T",tz="GMT")
True_heading<-transform(Mytable$V4,class=as.double(as.character(Mytable$V4)))
heading<-transform(Mytable$V4,class=as.double(as.character(Mytable$V4)))
ggplot(data=Mytable,aes(Time_,True_heading$class))+geom_line(color="Blue",aes(group=1)) +theme(axis.text.x = element_text(angle = 0),axis.text.y = element_text(angle = 0))+ggtitle(paste("True Heading vs Time","(from ", min_t, " to ",max_t,")" ))+ scale_x_datetime(labels=date_format("%H:%M"),breaks=date_breaks("60 min"),name ="Time in UTC")+scale_y_continuous(name ="True heading (Degrees)",limits=c(0,360), breaks=seq(0,360, by = 20))+theme(panel.grid.minor = element_line(colour = "gray", linetype = "dotted",size=.5))
...
ggplot(data=Mytable,aes(Time_,TMG$class))+geom_line(color="Blue",aes(group=10)) +theme(axis.text.x = element_text(angle = 0),axis.text.y = element_text(angle = 0))+ggtitle(paste("Course Made good vs Time","(from ", min_t, " to ",max_t,")" ))+ scale_x_datetime(labels=date_format("%H:%M"),breaks=date_breaks("60 min"),name ="Time in UTC")+theme(panel.grid.minor = element_line(colour = "gray", linetype = "dotted",size=.5))+scale_y_continuous(name ="Track Made(Degree)",limits=c(0,360), breaks=seq(0,360, by = 20))
...
p=ggplot(data=Mytable,aes(Time_,ROT$class, fill=positive))+geom_bar(stat='identity', position = 'identity')+theme(axis.text.x = element_text(angle = 0),axis.text.y = element_text(angle = 0),legend.position='none')+ggtitle(paste("Rate of Turn vs Time","(from ", min_t, " to ",max_t,")" ))+ scale_x_datetime(labels=date_format("%H:%M"),breaks=date_breaks("60 min"),name ="Time in UTC")+scale_y_continuous(name ="Rate of Turn (Degree/min)", limits=c(-5,5), breaks=seq(-5,5, by = .5))
grob1 <- grobTree(textGrob("Starboard", x=0.1,  y=0.95, hjust=0,
                           gp=gpar(col="Blue", fontsize=13, fontface="italic")))
grob2 <- grobTree(textGrob("Port", x=0.1,  y=.95, hjust=0,vjust = 17,
                           gp=gpar(col="red", fontsize=13, fontface="italic")))
A<-p+ theme(panel.grid.minor = element_line(colour = "gray", linetype = "dotted",size=.5))+annotation_custom(grob1)+annotation_custom(grob2)+geom_hline(yintercept=0)
ggplotly(A)
...
ggmap(mapamit)+geom_point(data = Mytable, aes(x = Mytable$longitude, y = Mytable$latitude, fill = "green", alpha = 0.01), size = .05, shape = 21) +
  guides(fill=FALSE, alpha=FALSE, size=FALSE)+geom_point(data = head_cord, aes(x = head_cord$longitude, y = head_cord$latitude, fill = "green", alpha = 0.1), size = 6, shape = 21) +
  guides(fill=FALSE, alpha=FALSE, size=FALSE)+geom_point(data = tail_cord, aes(x = tail_cord$longitude, y = tail_cord$latitude, fill = "Yello", alpha = 0.1), size = 6, shape = 21) +
...
temptable = sqldf('Select V1,V6*(case WHEN V6>10 THEN -1  WHEN V6 <10 THEN 1 END) V6 from Data where V3 like "%ETL"')
Mytable =sqldf('Select V1,(case WHEN V6< 0 THEN V6+10  WHEN V6 >=0 THEN V6 END) V6 from temptable')
